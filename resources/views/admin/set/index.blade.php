         ...
         <div class="col-6">
           <div class="w3-card-2">
             ...
             <div class="w3-container" style="margin-top: 10px">
              ...
              <input type="hidden" id="id_input" name="tea[]" value=" {{$employ->id}} ">
              <input data-id="{{$employ->id}}" data-config="Resource" value="{{$employ->id}}" class="toggle-class w3-check metric-config" type="checkbox" data-onstyle="success" data-offstyle="danger" data-toggle="toggle" name="teastatus[]" @if($employ->status) checked @endif id='status' ><label>{{ $employ->Metricname }} </label><br/>
              @endforeach
            </div>
          </div>
        </div>
